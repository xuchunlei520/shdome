...
service = item.get("services", {}).get("app", {})
need(bool(re.fullmatch(r"[a-zA-Z0-9][a-zA-Z0-9._/-]*:[a-zA-Z0-9][a-zA-Z0-9._-]*", str(service.get("image", "")))), "镜像必须使用固定 tag")
...
for volume in item.get("volumes", []):
    ...
    need(target.startswith("/") and ".." not in target.split("/"), "卷 target 必须是安全绝对路径")
