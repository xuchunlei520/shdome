...
shdome_config_init() {
    ...
    : "${SHDOME_CATALOG_ROOT:=$SHDOME_ROOT/catalog}"
    : "${SHDOME_CUSTOM_CATALOG_DIR:=$SHDOME_CATALOG_ROOT/custom}"
    : "${SHDOME_OFFICIAL_CATALOG_ROOT:=$SHDOME_CATALOG_ROOT/official}"
    : "${SHDOME_OFFICIAL_RELEASES_DIR:=$SHDOME_OFFICIAL_CATALOG_ROOT/releases}"
    : "${SHDOME_CATALOG_REFRESH_INTERVAL:=86400}"
    : "${SHDOME_IMAGE_SOURCE_CONFIG:=$SHDOME_CONFIG_DIR/image-sources.json}"
    : "${SHDOME_IMAGE_SOURCE_STATE:=$SHDOME_STATE_DIR/image-source-health.json}"
    ...
    export SHDOME_CATALOG_DIR SHDOME_CATALOG_ROOT SHDOME_CUSTOM_CATALOG_DIR
    export SHDOME_OFFICIAL_CATALOG_ROOT SHDOME_OFFICIAL_RELEASES_DIR SHDOME_CATALOG_REFRESH_INTERVAL
    export SHDOME_IMAGE_SOURCE_CONFIG SHDOME_IMAGE_SOURCE_STATE
    export SHDOME_GITHUB_REPO SHDOME_RELEASE_VERSION
    ...
    if [[ ${EUID:-$(id -u)} -eq 0 || -w "$SHDOME_ROOT" || "${SHDOME_ALLOW_NON_ROOT:-0}" == "1" ]]; then
        mkdir -p "$SHDOME_CONFIG_DIR" "$SHDOME_APPS_DIR" "$SHDOME_STATE_DIR" \
            ...
            "$SHDOME_CUSTOM_CATALOG_DIR" "$SHDOME_OFFICIAL_RELEASES_DIR"
        chmod 750 "$SHDOME_CATALOG_ROOT" "$SHDOME_CUSTOM_CATALOG_DIR" \
            "$SHDOME_OFFICIAL_CATALOG_ROOT" "$SHDOME_OFFICIAL_RELEASES_DIR" 2>/dev/null || true
    fi
}
